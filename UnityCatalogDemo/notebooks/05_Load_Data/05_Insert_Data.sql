...
-- MAGIC %md
-- MAGIC # Notebook 5 — Loading Data
-- MAGIC
-- MAGIC ## Explanation
-- MAGIC There are two common ways to load data into Unity Catalog tables:
-- MAGIC 1. **`INSERT INTO ... VALUES`** — fine for small reference data or a live demo (used below).
-- MAGIC 2. **`COPY INTO`** reading from a **Volume** — the production pattern for loading files
-- MAGIC    (CSV/JSON/Parquet) that land in cloud storage. We demonstrate this in Notebook 10 (Volumes) once
-- MAGIC    the Volume concept has been introduced.
-- MAGIC
-- MAGIC The source data for all `INSERT` statements below mirrors the CSV files in `/data` in this repo
-- MAGIC (`customers.csv`, `products.csv`, `orders.csv`, `sales.csv`, `employees.csv`, `departments.csv`).
...
-- MAGIC Each `INSERT` reports `num_affected_rows`. Verify row counts:
...
-- MAGIC - Because these are **Delta** managed tables, every `INSERT` is ACID and immediately versioned
-- MAGIC   (`DESCRIBE HISTORY retail_sales.sales.customers` shows the commit).
-- MAGIC - In production you'd replace this notebook with `COPY INTO` or a Delta Live Tables / Lakeflow
-- MAGIC   pipeline reading from a Volume — shown in Notebook 10.
