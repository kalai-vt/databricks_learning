...
interface LogRow {
  ...
  action: string; model: string | null;
}
...
export default function AuditLogs() {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-4">
      <div>
        <h1 className="text-lg font-bold text-slate-900 flex items-center gap-2"><ScrollText size={20} /> Audit Logs</h1>
        <p className="text-sm text-slate-500">Every AI request — allowed or blocked — is recorded here for full accountability.</p>
      </div>
          ...
          <tbody className="divide-y divide-slate-100">
            {filtered.map((l) => (
              <tr key={l.id}>
                ...
                <td className="p-3 text-xs text-slate-500">{l.role}</td>
                ...
                <td className="p-3 text-xs text-slate-400">{l.model ?? "—"}</td>
              </tr>
            ))}
            {filtered.length === 0 && (
              <tr><td colSpan={9} className="p-6 text-center text-slate-400">No audit entries match this filter.</td></tr>
            )}
          </tbody>
        </table>
      </div>
